...
impl Config {
    ...
    fn probe_pkg_config(&mut self) -> Result<Dependencies, Error> {
        ...
        for dep in metadata.deps.iter() {
            ...
            let pkg_config_paths: Option<[&str; 0]> = None;
            #[cfg(feature = "binary")]
            let pkg_config_paths = self
                .env
                .get(&EnvVariable::new_no_binary(Some(name)))
                .or(self.env.get(&EnvVariable::new_no_binary(None)))
                .map_or_else(
                    || Some([get_path(name.as_str()), get_path("")].concat()),
                    |_| None,
                );
            ...
            let mut library = if self.env.contains(&EnvVariable::new_no_pkg_config(name)) {
                ...
                let probe = match pkg_config_paths {
                    Some(p) => Library::wrap_pkg_config(&p, || {
                        Self::probe_with_fallback(&config, lib_name, fallback_lib_names)
                    }),
                    None => Self::probe_with_fallback(&config, lib_name, fallback_lib_names),
                };
